...
color_vec = [
    ...
    "#7E2F8E"  % purple
    "#EDB120"  % gold / yellow
    "#77AC30"  % lime green (distinct from main green)
    "#C00000"  % bright red (contrast with dark red)
    "#5F9EA0"   % teal / muted cyan
];
...
% options: "ext_oma_cwe_joint"
experiment = "ext_oma_cwe_joint";
disp("Plotting " + experiment)

cwe_joint_1 = load('ext_cwe_joint_1_results.mat');
cwe_joint_2 = load('ext_cwe_joint_2_results.mat');
oma_joint_1 = load('ext_oma_joint_1_results.mat');
oma_joint_2 = load('ext_oma_joint_2_results.mat');

cwe_joint_avg_dep_var = mean(cat(7, cwe_joint_1.empirical_var, cwe_joint_2.empirical_var), 7);
cwe_joint_avg_dep_mse = mean(cat(7, cwe_joint_1.empirical_mse, cwe_joint_2.empirical_mse), 7);
cwe_joint_avg_dep_crlb = mean(cat(7, cwe_joint_1.crlb, cwe_joint_2.crlb), 7);

oma_joint_avg_dep_var = mean(cat(7, oma_joint_1.empirical_var, oma_joint_2.empirical_var), 7);
oma_joint_avg_dep_mse = mean(cat(7, oma_joint_1.empirical_mse, oma_joint_2.empirical_mse), 7);
oma_joint_avg_dep_crlb = mean(cat(7, oma_joint_1.crlb, oma_joint_2.crlb), 7);
...
for agent_db_idx = 1:length(agent_db_values)
    ...
    for param_idx = 1:2
        ...
        all_vals = [cwe_joint_avg_dep_var(:,agent_db_idx,:,param_idx,selected_scheme_idx,1:sensor_dimension);...
                    cwe_joint_avg_dep_mse(:,agent_db_idx,:,param_idx,selected_scheme_idx,1:sensor_dimension);...
                    cwe_joint_avg_dep_crlb(:,agent_db_idx,:,param_idx,selected_scheme_idx,1:sensor_dimension);
                    oma_joint_avg_dep_var(:,agent_db_idx,:,param_idx,selected_scheme_idx,1:sensor_dimension);...
                    oma_joint_avg_dep_mse(:,agent_db_idx,:,param_idx,selected_scheme_idx,1:sensor_dimension);...
                    oma_joint_avg_dep_crlb(:,agent_db_idx,:,param_idx,selected_scheme_idx,1:sensor_dimension)];
        ...
        for scheme_idx = selected_scheme_idx % 1:length(selected_schemes)
            ...
            for count_idx = 1:length(count_vals)
                ...
                plot(x_axis_series,(squeeze(cwe_joint_avg_dep_var(:,agent_db_idx,:,param_idx,scheme_idx,count_idx,1))),'-^','color',color_vec(2*count_idx-1),'LineWidth', 1)
                plot(x_axis_series,(squeeze(cwe_joint_avg_dep_mse(:,agent_db_idx,:,param_idx,scheme_idx,count_idx,1))),'-x','color',color_vec(2*count_idx-1),'LineWidth', 1)
                plot(x_axis_series,(squeeze(cwe_joint_avg_dep_crlb(:,agent_db_idx,:,param_idx,scheme_idx,count_idx,1))),'-o','color',color_vec(2*count_idx-1),'LineWidth', 1)
                legend_entries = [legend_entries, "CWE, S = " + count_vals(count_idx)];

                plot(x_axis_series,(squeeze(oma_joint_avg_dep_var(:,agent_db_idx,:,param_idx,scheme_idx,count_idx,1))),'--^','color',color_vec(2*count_idx),'LineWidth', 1)
                plot(x_axis_series,(squeeze(oma_joint_avg_dep_mse(:,agent_db_idx,:,param_idx,scheme_idx,count_idx,1))),'--x','color',color_vec(2*count_idx),'LineWidth', 1)
                plot(x_axis_series,(squeeze(oma_joint_avg_dep_crlb(:,agent_db_idx,:,param_idx,scheme_idx,count_idx,1))),'--o','color',color_vec(2*count_idx),'LineWidth', 1)
                legend_entries = [legend_entries, "OMA, S = " + count_vals(count_idx)];
            end
            ...
            if param_idx == 1
                ...
                y_upper = 5*max(all_vals,[],"all");
            else
                y_lower = 10^(-0.25)*min(all_vals,[],"all");
                y_upper = 100*max(all_vals,[],"all");
            end
            ...
            if (what_to_plot == "all" && scheme == "MPC") || (what_to_plot == "EPC" && scheme == "EPC") 
                lgd1 = legend(h_colors, cellstr(legend_entries), 'Location', 'north east', 'NumColumns', 2);
                ...
                lgd2 = legend(ax2, h_symbols, cellstr(["VAR","MSE","CRLB"]), 'Location', 'north east', 'Color', 'white', 'FontSize', 13.5);
                ...
                new_x = lgd2_pos(1);
                new_y = lgd1_pos(2) - lgd1_pos(4) + 0.04;
